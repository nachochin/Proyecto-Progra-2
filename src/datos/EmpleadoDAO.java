 ...
 * {@link BufferedReader} y {@link BufferedWriter}. Cada línea del archivo
 * representa un empleado con el formato:
 * </p>
 * <pre>
...
public class EmpleadoDAO implements ManejadorArchivo<Empleado, String> {
     ...
     *
     * @return Lista de todos los empleados registrados. Lista vacía si el
     ...
     * <p>
     * Lee todas las líneas, reemplaza la que coincide con el ID del empleado y
     ...
     * <p>
     * Lee todas las líneas, omite la que coincide con el ID y reescribe el
     * archivo sin esa línea.
     * </p>
     *
    ...
    // -------------------------------------------------------------------------
    /**
     * Convierte un objeto {@link Empleado} a una línea de texto con formato
     * pipe.
     *
     * @param e Empleado a convertir.
     * @return Línea con formato:
     * {@code id|nombre|correo|salarioBase|tipoContrato|fechaIngreso}
     */
     ...
     * @throws ArchivoInvalidoException si la línea no tiene el número de campos
     * esperado o si algún dato tiene un tipo incorrecto.
     */
    ...
    /**
     * Genera el siguiente ID disponible basado en el último ID registrado. Si
     * no hay empleados, retorna "01".
     *
     * @return Siguiente ID disponible con formato de dos dígitos (01, 02, ...).
     * @throws ArchivoInvalidoException si ocurre un error al leer el archivo.
     */
    public String generarSiguienteId() throws ArchivoInvalidoException {
        List<Empleado> lista = listarTodos();
        if (lista.isEmpty()) {
            return "01";
        }
        ...
        for (Empleado e : lista) {
            try {
                int num = Integer.parseInt(e.getId().trim());
                if (num > maxId) {
                    maxId = num;
                }
